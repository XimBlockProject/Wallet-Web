...
namespace BitcoinLib.Services
{
    ...
    public partial class CoinService : ICoinService
    {
        ...
        public string GetNewAddress(string account)
        {
			...
			public string CreateNewSegwitAddressFromNormalAddress(string normalAddress, string account)
			{
				var newSegwitAddress =
					_rpcConnector.MakeRequest<string>(RpcMethods.addwitnessaddress, normalAddress);
				// Explicitly set the account label to the new segwit address to we easily see the user id
				_rpcConnector.MakeRequest<string>(RpcMethods.setaccount, newSegwitAddress, account);
				return newSegwitAddress;
